...
class EnsembleDetector:
    def __init__(self, baseline_weights_path="models/baseline_weights.pth", clip_weights_path="models/clip_vit_l_weights.pth", device=None):
        self.device = device or torch.device("mps" if torch.backends.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu")
        
        # Константы калибровки (Temperature Scaling) - НОВЫЕ ЗНАЧЕНИЯ
        self.T_convnext = 1.7018
        self.T_clip = 1.4915
        ...
        # 2. CLIP (ОБНОВЛЕНО НА Тяжелый ViT-L/14)
        self.clip_model, self.clip_preprocess = clip.load("ViT-L/14", device=self.device)
        self.clip_model.eval()
        
        # Размерность изменена на 768
        self.clip_probe = nn.Linear(768, 1).to(self.device)
    ...
    def predict(self, image_path, mode="uncertainty", threshold=0.50, baseline_weight=0.5, clip_weight=0.5):
        raw_image = Image.open(image_path).convert("RGB")
        
        # Инференс ConvNeXt с калибровкой
        img_cnn = self.cnn_transform(raw_image).unsqueeze(0).to(self.device)
        with torch.no_grad():
            raw_cnn_logit = self.baseline(img_cnn)
            cnn_logit = raw_cnn_logit / self.T_convnext
            cnn_prob = torch.sigmoid(cnn_logit).item()

        # Инференс CLIP с калибровкой
        ...
        with torch.no_grad():
            ...
            raw_clip_logit = self.clip_probe(features)
            clip_logit = raw_clip_logit / self.T_clip
            clip_prob = torch.sigmoid(clip_logit).item()

        # Uncertainty-Weighted Fusion на откалиброванных вероятностях
        if mode == "uncertainty":
