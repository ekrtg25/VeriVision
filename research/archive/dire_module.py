...
class DIREAnalyzer:
    ...
    def get_reconstruction_error(self, image_path, num_inference_steps=20):
        ...
        original_tensor = self.transform(raw_img).unsqueeze(
            0).to(self.device, dtype=self.pipe.dtype)
        ...
        with torch.no_grad():
            ...
            timesteps = torch.tensor(
                [self.pipe.scheduler.config.num_train_timesteps - 1], device=self.device)
