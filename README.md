...
<!-- МЕСТО ДЛЯ ГЛАВНОГО DEMO GIF/СКРИНШОТА ИНТЕРФЕЙСА -->
<!-- Сохранить в docs/media/hero_demo.gif и раскомментировать -->
![VeriVision MoE v3.5 Demo](docs/media/hero_demo.gif)
...
Современные детекторы генеративного контента сталкиваются с тремя фундаментальными барьерами:

1. **Shortcut Learning (Зубрёжка артефактов датасета):** Модели на базе стандартных ResNet/EfficientNet цепляются за пиксельный шум и профиль сжатия конкретной обучающей выборки. При переносе на чистые OOD-генерации (Midjourney v6, Flux) точность таких моделей падает до уровня случайного угадывания ($\approx 50\%$).
2. **Ложные срабатывания на сжатии мессенджеров:** Агрессивный JPEG-рекомпресс (Telegram, WhatsApp) разрушает высокочастотные спектры реальных фотографий. Классическая форензика (ELA, FFT) ошибочно принимает это за следы апскейлеров или Inpainting.
3. **Black-Box вердикты:** Отсутствие объяснимости делает невозможной экспертную проверку спорных зон изображения (локальные генеративные вклейки и фейсинг).

**VeriVision MoE v3.5** решает эти проблемы разделением анализа на независимые модальности (семантическую и физическую) с динамической регулировкой доверия в зависимости от степени сжатия файла.
...
Перед запуском тяжелого инференса Zero-Shot семантический фильтр проверяет, является ли изображение цифровой графикой, 3D-рендером или скриншотом. Это изолирует нерелевантные кейсы без ложных обвинений физических экспертов.

### 2. Fine-Tuned DINOv2 Dense Student (1536d)
* **Бэкбоун:** `facebook/dinov2-base` (ViT-B/14, 86M параметров), обученный на self-supervised задаче DINOv2 и инвариантный к случайным цветовым искажениям.
* **Partial Fine-Tuning:** Разморожены 2 верхних блока трансформера (`encoder.layer.10`, `encoder.layer.11`) и финальный `layernorm`. Использован дифференциальный LR: $1.5 \cdot 10^{-5}$ для блоков внимания и $2.0 \cdot 10^{-4}$ для классификационной головы.
* **Мультимасштабное представление:** Классификатор принимает конкатенацию CLS-токена ($768$) и среднего пространственного вектора патчей ($768$), образуя вектор признаков размерностью **$1536$**.
* **Dense Patch Evaluation:** Параллельный прогон классификатора по всем $1369$ патчам ($37 \times 37$ сетка при $518\text{px}$) генерирует карту аномалий локального деноайзинга без использования ресурсоемкого Grad-CAM.

### 3. Физический уровень: PRNU, ELA, 2D FFT & SRM
* **PRNU Sensor Trace:** Извлечение остаточного шума кремниевой матрицы фотокамеры (Photo-Response Non-Uniformity). ИИ-генераторы не обладают сенсорным шумом оптического тракта.
* **Error Level Analysis (ELA):** Анализ градиентов матриц квантования JPEG при контролируемом пересохранении (качество 90). Позволяет обнаружить локальный Inpainting и вклейки.
* **2D FFT Spectrum:** Поиск радиальных частотных сеток и гармонических пиков, оставляемых латентными декодерами VAE и сверточными апскейлерами.
...
Классические эксперты не суммируются жестким весовым средним:
1. **Dynamic Gating:** При падении оцениваемого качества JPEG-компрессии вес ELA и FFT линейно глушится:
   $$\text{Weight}_{\text{ELA, FFT}} \leftarrow \text{Weight}_{\text{raw}} \times \max(0.1, \min(1.0, Q_{\text{comp}}))$$
2. **Weight Capping:** Максимальный вес классических экспертов жестко ограничен $\le 20\%$ от веса DINOv2. Это предотвращает подавление семантической модели спектральным шумом.
3. **Platt Calibration:** Сырые расстояния и скоры каждого эксперта независимо калибруются через сигмоидальную регрессию (`CalibratorBank`).

---

## 🔍 Explainable AI (XAI) & Heatmaps

<!-- МЕСТО ДЛЯ СКРИНШОТОВ КАРТОЧЕК И ТЕПЛОВЫХ КАРТ -->
<!-- Сохранить скриншоты 4 карточек в docs/media/experts_cards.png -->
![Explainable AI Breakdown](docs/media/experts_cards.png)
...
* **2D FFT Log-Magnitude:** Двумерная спектрограмма распределения высоких и низких пространственных частот.
...
| 4 | 1D Radial FFT Profile + Random Forest | Recall Fake 84%, но Recall Real 67% | ✅ Высокий recall на фейках, но огромный False Positive на реальных фото |
...
* **Serving:** Асинхронный сервер на базе **FastAPI + Uvicorn** с кастомными эндпоинтами потокового форензик-анализа (`/api/analyze` и `/api/deep-analysis`).
* **Аппаратное ускорение:** Полная поддержка инференса на **NVIDIA CUDA** и **Apple Silicon (MPS)** с автоматическим fallback на CPU.
* **Mixed Precision (AMP FP16):** Использование `torch.amp` позволило ускорить прогон трансформера ViT-B/14 на Kaggle/сервере в 3 раза при минимальном потреблении VRAM.
* **UI/UX:** Минималистичный швейцарский дизайн (Bento Grid, JetBrains Mono типографика, Glassmorphism, CSS Custom Properties) на шаблонизаторе Jinja2 без тяжелых клиентских фреймворков.
...
```bash
git clone [https://github.com/your-username/VeriVision.git](https://github.com/your-username/VeriVision.git)
cd VeriVision
...
├── docs/                       # Архитектурные диаграммы и медиа
